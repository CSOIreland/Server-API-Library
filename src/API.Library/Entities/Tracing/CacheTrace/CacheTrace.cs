...
namespace API
{
    ...
    internal class CacheTrace
    {
        internal static DataTable CreateCacheTraceDataTable()
        {
            ...
            dataTable.Columns.Add("CCH_CACHE_LOCK_DURATION", typeof(SqlDecimal));
            dataTable.Columns.Add("CCH_CACHE_LOCK_USED");
            return dataTable;
        }


        //adds rows to cacheTraceDataTable asynclocal variable
        internal static void PopulateCacheTrace(string cacheObject, DateTime startTime, decimal duration, string action, bool success, int? compressed_size, DateTime? expiresAt, decimal? cacheLockDuration = null, bool cacheLockUsed = false)
        {
            if (ApiServicesHelper.ApiConfiguration != null && APIMiddleware.correlationID.Value != null)
            {
                ...
                {
                    APIMiddleware.cacheTraceDataTable.Value.Rows.Add(APIMiddleware.correlationID.Value, cacheObject, startTime, duration, action, success, compressed_size, expiresAt, cacheLockDuration, cacheLockUsed);
                }
            }
